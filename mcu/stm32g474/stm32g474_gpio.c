...
{
    port_reg->MODER = ((port_reg->MODER & ~(3UL << (pin * 2)))
        | ((uint32_t)mode << (pin * 2)));
}


static void gpio_init_output_type(GPIO_TypeDef* port_reg, uint8_t pin, gpio_output_type_t output_type)
{
    port_reg->OTYPER &= (port_reg->OTYPER & (~(1UL << pin))
        | ((uint32_t)output_type << pin));
}


static void gpio_init_output_speed(GPIO_TypeDef* port_reg, uint8_t pin, gpio_speed_t speed)
{
    port_reg->OSPEEDR &= ((port_reg->OSPEEDR & ~(3UL << (pin * 2)))
...
{
    port_reg->PUPDR &= ((port_reg->PUPDR & ~(3UL << (pin * 2))) |
        ((uint32_t)pull << (pin * 2)));
}
...
static void gpio_init_af(GPIO_TypeDef* port_reg, uint8_t pin, gpio_af_t af)
{
    ...
    port_reg->AFR[idx] &= ((port_reg->AFR & ~(15U << shift)) |
        ((uint32_t)af << shift));
}
...
{
    port_reg->LCKR =
    ((port_reg->LCKR & ~(1U << GPIO_LCKR_LCKK_Pos)) |
...
)
{
    ...
    switch (port)
    {
    case GPIO_PORT_A: port_reg = GPIOA;
        break;
    case GPIO_PORT_B: port_reg = GPIOB;
        break;
    case GPIO_PORT_C: port_reg = GPIOC;
        break;
    case GPIO_PORT_D: port_reg = GPIOD;
        break;
    case GPIO_PORT_E: port_reg = GPIOE;
        break;
    case GPIO_PORT_F: port_reg = GPIOF;
        break;
    case GPIO_PORT_G: port_reg = GPIOG;
        break;
    }
